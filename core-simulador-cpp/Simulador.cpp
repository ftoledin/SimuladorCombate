...
Personagem* Simulador::proximoPersonagem(vector<Personagem*> equipe)
{
    ...
    int rngpersonagem = rand() % tamanho;
    if(equipe[rngpersonagem]->getVida() > 0){
        return equipe[rngpersonagem];
    }
    while (equipe[rngpersonagem]->getVida() <= 0){
        rngpersonagem = rand() % tamanho;
    }
